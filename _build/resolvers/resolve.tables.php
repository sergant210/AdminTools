...
if ($object->xpdo) {
    ...
    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
        ...
        case xPDOTransport::ACTION_UPGRADE:
            $modelPath = $modx->getOption('admintools_core_path', null, $modx->getOption('core_path') . 'components/admintools/') . 'model/';
            $modx->addPackage('admintools', $modelPath);

            $manager = $modx->getManager();
            $objects = array();
            $schemaFile = MODX_CORE_PATH . 'components/admintools/model/schema/admintools.mysql.schema.xml';
            if (is_file($schemaFile)) {
                ...
                $modx->log(1,"Error of loading xml schema");
            }
